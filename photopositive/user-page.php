...
?>
<?php
// Странца личного кабинета пользователя
if ( is_user_logged_in() ) { // проверяем зарегистрированный это пользователь или нет
...
// обрабатываем запрос на изменение аватарки
if(isset($_POST)) {
	if(isset($_FILES['avatar'])) {
		// echo "Меняем аватарку";
		...
		// echo $img_tag;
		//
		$meta_key = 'wp_foto_user_avatar'; // поле для создания или изменения
		// $prev_value = ''; предыдущее значения поля если было
		update_user_meta($current_user_id, $meta_key, $img_tag);
	}
}
// обрабатываем запрос на удаление фотографии
if(isset($_POST)) {
	if(isset($_POST['post-id']) && isset($_POST['del-post'])) {
		if($_POST['del-post'] == 'del-post') {
			...
			$force_delete = true; // удаляем мимо корзины
			wp_delete_post( $postid, $force_delete );
		}
	}
}
// обрабатываем ПОСТ запросы с формы добавления новой фотографии
if(isset($_POST)) {
	// создаем новый пост и добавляем его в текущую рубрику
	// получаем ID нового поста
	// добавляем изображение к посту с текущим ID
	if(isset($_POST['comment'])) {
		...
		$post_id = wp_insert_post( $post_data ); // получаем ID поста для добавления фотографии
		// echo "ID: ".$post_id;
	}
	if(isset($_FILES['f'])) {
		// print_r($_FILES['f']);
		...
		require_once ABSPATH . 'wp-admin/includes/image.php';
		$file_array = $_FILES['f'];
		$desc = $_POST['comment']; // можно задать другое название картинки для библиотеки
		$post_data = array(
			'post_parent'    => $post_id,
		);
		// echo $post_id;
		...
		if( is_wp_error($img_tag) ){
			...
			// echo "добавлено".$img_tag;
			// echo "Путь к изображению: ".wp_get_original_image_path($img_tag);
			set_post_thumbnail( $post_id, $img_tag );
		}
	}
}
// обрабатываем пост запросы с формы user (ФИО, телефон, email, пароль, новый пароль, повтор пароля)
if(isset($_POST)) {
	...
	if(isset($_POST['new-pass']) && isset($_POST['user-submit']) && isset($_POST['rep-new-pass']) && isset($_POST['current-pass'])) {
		// проверяем совпадение предыдущего пароля
...
// запрашиваем данные пользователя для отображения в форме с возможностью изменения
      ...
      <div class="user-container">
        <div class="info">
          ...
          <div class="info__line">
            <div class="step-1"><div class="step-2"><div class="step-3"><div class="step-4"><div class="step-5"><div class="step-6"><div class="step-7"></div></div></div></div></div></div></div>
          </div>
        </div>
        <div class="exit">
      ...
      <div class="flex">
        ...
        <div class="flex_info">
          ...
          <div class="d-none user">
						...
						// echo $username = $user_data->get('user_login');
						// запрашиваем номер телефонона если есть
						$user_tel = $wpdb->get_results("SELECT tel FROM wp_users_tel WHERE user_id=$current_user_id");
						if(empty($user_tel)) { unset($user_tel);} else {print_r($user_tel);}

						?>
            <form class="user-form" action="" method="post">
              <p>Только при наличии актуальных данных Вы можете получить выигрыш. Призовые деньги не передаются по фейковым данным. </p>
							<input type="text" name="user-nick" value="<?php echo $user_data->get('nickname'); ?>" placeholder="Ник">
              <input type="text" name="last-name" value="<?php echo $user_data->get('last_name'); ?>" placeholder="ФИО">
              <input type="tel" name="user-tel" value="<?php echo $user_data->get('user_login'); ?>" placeholder="Номер телефона" disabled>
              <!-- <input type="email" name="user-email" value="<?php /* echo $user_data->get('user_email'); */ ?>" placeholder="Адрес электронной почты"> -->
            ...
            <div class="image-content">
              <div class="image-field">
                ...
                <div class="space"></div><!-- Пустой блок для отступа первого изображения -->
								...
								<?php
								// формирование запроса для вывода списка работ
								$args = array(
									'posts_per_page' => 10, // лимит постов
								 );
										...
										<div class="card">
											<div class="card__image">
												<form class="del-foto" action="" method="post">
													...
													<button type="submit" name="del-post" value="del-post" title="Удалить"><i class="far fa-times-circle"></i></button>
												</form>
              ...
              <div class="image-button">
								<form class="d-none" action="https://pv-foto.ru/%d0%bb%d0%b8%d1%87%d0%bd%d1%8b%d0%b9-%d0%ba%d0%b0%d0%b1%d0%b8%d0%bd%d0%b5%d1%82/" method="post" id="go-to-add-foto"></form>
                <button type="submit" name="go-to-add-foto" value="3" form="go-to-add-foto">Добавить</button>
              </div>
            </div>
          </div>
          ...
          <div class="d-none work">
            ...
            <form class="foto-form" action="" enctype="multipart/form-data" method="post">
              ...
              <p>
                ...
                Использование чужих фото, а также декораций, созданных другими участниками, влечет за собой прекращение участия в проекте.
              </p>
							<input type="hidden" name="go-to-add-foto" value="2">
